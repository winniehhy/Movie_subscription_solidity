...
async function loadProfile(account) {
  try {
    const isRegistered = await userManager.methods
      ...
      .call();
    if (isRegistered) {
      const profile = await userManager.methods
        .getProfile()
        .call({ from: account });
      ...
      if (nextUpdateTime > 0) {
        document.getElementById("nextUpdateTime").innerText = new Date(
          nextUpdateTime * 1000
        ).toLocaleString();
      } else {
        document.getElementById("nextUpdateTime").innerText =
          "Not yet updated";
      }
    } else {
    ...
    alert(
      "You are not registered. Please register before accessing this page."
    );
...
document
  .getElementById("showProfileModal")
  .addEventListener("click", async () => {
    const accounts = await web3.eth.getAccounts();
    if (accounts.length > 0) {
      loadProfile(accounts[0]);
      document.getElementById("currentProfileModal").style.display =
        "block";
    } else {
...
document
  .getElementById("closeCurrentModal")
  .addEventListener("click", () => {
    document.getElementById("currentProfileModal").style.display = "none";
  });

document
  .getElementById("closeModifiedModal")
  .addEventListener("click", () => {
    document.getElementById("modifiedProfileModal").style.display =
      "none";
  });
  ...
  .addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (nextUpdateTime === 0 || currentTime >= nextUpdateTime) {
        ...
        document.getElementById("modifiedProfileModal").style.display =
          "block";
        document.getElementById("modifiedUsername").textContent =
          newUsername;
